...
exports.addPlant = async (req, res) => {
  try {
    const { name, price, category, description, stock, care, featured } = req.body;
...
exports.getAllPlants = async (req, res) => {
  console.log('getAllPlants route hit');
  try {
...
exports.getCategories = async (req, res) => {
  try {
    const categories = await Plant.distinct('category'); // or filter with { featured: false }
    res.json(categories);
  } catch (error) {
...
exports.getFeatured = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch featured product', error });
  }
};
